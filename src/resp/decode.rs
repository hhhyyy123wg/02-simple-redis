...
mod tests {
    ...
    fn test_simple_string_decode() -> Result<()> {
        let buf = b"+OK\r\n";
        let mut bytes_mut = BytesMut::from(buf.as_slice());
        let frame = SimpleString::decode(&mut bytes_mut)?;
        assert_eq!(frame, SimpleString::new("OK"));

        bytes_mut.extend_from_slice(b"+hello\r");
        let ret = SimpleString::decode(&mut bytes_mut);
